...
def student_signup_view(request):
    ...
    if request.method=='POST':
        ...
        if userForm.is_valid() and studentForm.is_valid():
            ...
            mydict['invalid'] = True
            return render(request, 'student/studentsignup.html', mydict)
    return render(request,'student/studentsignup.html', mydict)
...
def start_exam_view(request,pk):
    ...
    if request.method=='POST':
        pass
    response= render(request,'student/start_exam.html',{'course':course,'questions':questions, 'range':len(questions)})
...
def calculate_marks_view(request):
    if request.COOKIES.get('course_id') is not None:
        ...
        result= QMODEL.Result.objects.filter(student_id=request.user.id)
