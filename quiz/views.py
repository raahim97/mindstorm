...
def admin_add_question_view(request):
    ...
    if request.method == 'POST' and 'document' in request.FILES:
        ...
        csv_reader = uploaded_file.read().decode('ISO-8859-1')
        io_string = io.StringIO(csv_reader)
        next(io_string)
        data_list = []
        for column in csv.reader(io_string):
            questionn.append(column[0])
            option1.append(column[1])
            option2.append(column[2])
            option3.append(column[3])
            option4.append(column[4])
            answer.append(column[5])
        
        # for singledata in uploaded_file:
        #     uploadedFileWithoutStr = '{}'.format(singledata.strip()).replace("b\'", '').replace("\'", '')
        #     data_list.append(uploadedFileWithoutStr)
        xyz = zip(questionn, option1, option2, option3,option4, answer)
        context = {
            'uploadedFileWithoutStr':data_list,
            'xyz':xyz,
            'questionForm':questionForm
        }

        return render(request,'quiz/admin_add_question.html', context)

    if request.method=='POST':
        if 'file_submit' in request.POST:
            print('file sbumission button press')
            course=models.Course.objects.get(id=request.POST.get('courseID'))
            print(course)
            for n in questionn:
                print('main loop main agya')
                ques.course = course
                ques.question = questionn[n]
                ques.option1 = option1[n]
                ques.option2 = option2[n]
                ques.option3 = option3[n]
                ques.option4 = option4[n]
                ques.answer = answer[n]
                ques.save()
...
@login_required(login_url='adminlogin')
def admin_add_question_from_file_view(request):
    questionn = []
    option1 = []
    option2 = []
    option3 = []
    ...
    if request.method == 'POST':
        if 'document' in request.FILES:
            ...
            for column in csv.reader(io_string):
                ...
                option4.append(column[4])
                answer.append(column[5])
            data_dict['questions'] = questionn
            data_dict['option1'] = option1
            data_dict['option2'] = option2
            data_dict['option3'] = option3
            data_dict['option4'] = option4
            ...
            return render(request, 'quiz/admin_add_question_from_file.html', context)
            ...
            data = zip(data_dict['questions'], data_dict['option1'], data_dict['option2'], data_dict['option3'], data_dict['option4'], data_dict['answer'])
            for questionn, option1, option2, option3, option4, answer in data:
                print('main loop main agya')
                # ques.course = course
                # ques.question = questionn
            ...
            print('loop say bahir agya main')
            return HttpResponseRedirect('/admin-view-question')


    return render(request,'quiz/admin_add_question_from_file.html', {'questionForm':questionForm})
